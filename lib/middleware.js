...
function MiddlewareLogger(){
  ...
  middlewareLogger._levels = ['debug', 'log', 'info', 'warn', 'error'];
  ...
  ['debug', 'log', 'info', 'warn', 'error'].forEach(function(logLevel){
    middlewareLogger[logLevel] = function(){
      return middlewareLogger.execute.call(middlewareLogger, logLevel, arguments);
    }
  });
}

function useLogger(log, level){
  var middlewareLogger = this;
  var logger = {};
  level = level || 'debug';
  var mapLevel = false;
  middlewareLogger._levels.forEach(function(logLevel){
    if(!mapLevel && logLevel === level){
      mapLevel = true;
    }
    if(mapLevel){
