(function(module) {
  ...
  function registerDevice(apnToken, lastApnToken){
    var notifier = this;
    var lastToken = (lastApnToken===undefined) ? apnToken : lastApnToken;
    var fullId = notifier._options.datastore.idFromComponents(notifier._options.devicesPrefix, lastToken);
    ...
    return notifier._options.datastore.getDocument(fullId).then(
      function(doc) {
      console.log(doc)
      ...
      return notifier._updateDevice(apnToken, ops);  //status always ok b/c only active device registers
    },
    function(rejection){  //datastore always assumes no doc means client asked for something that's been deleted and rejects; for us it's just a new doc.
      if(rejection.isDeleted){
        notifier.log.info('Registering new device: ' + apnToken);
